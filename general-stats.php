...
function generalstats_wp_head() {
  echo("<meta name=\"GeneralStats\" content=\"0.11\"/>");
}
...
function GeneralStatsCounter($option) {
      ...
      switch ($option) {
        case 0:
        	$statement = "SELECT COUNT(*) as counter FROM $wpdb->users";
	        break;
        case 1:
          	$statement = "SELECT COUNT(*) as counter FROM $wpdb->categories";
        	break;
        case 2:
        	$statement = "SELECT COUNT(*) as counter FROM $wpdb->posts WHERE post_status = 'publish'";
	        break;
        case 3:
                $statement = "SELECT COUNT(*) as counter FROM $wpdb->comments WHERE comment_approved = '1'";
        	break;
        case 4:
                $statement = "SELECT COUNT(*) as counter FROM $wpdb->posts WHERE post_status = 'static'";
        	break;
        case 10:
        	$statement = "SELECT post_content FROM $wpdb->posts WHERE post_status = 'publish'";
        	break;
        case 11:
                $statement = "SELECT comment_content FROM $wpdb->comments WHERE comment_approved = '1'";
        	break;
        case 12:
                $statement = "SELECT post_content FROM $wpdb->posts WHERE post_status = 'static'";
        	break;
      }
